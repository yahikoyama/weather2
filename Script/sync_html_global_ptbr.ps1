# ==================================================
# sync_html_global_ptbr.ps1
# ポルトガル語（ブラジル）版HTML生成 + GitHubアップロード（完全版）
# ==================================================

$DictFile = "C:\weather\config\dictionary_ptbr.json"
...
$OutputFile = "C:\Users\winserverroot\OneDrive\weather_report_now_ptbr.html"
...
Write-Host "ポルトガル語（ブラジル）翻訳中..." -ForegroundColor Cyan

# -----------------------------
# 地名（長い語 → 短い語）
# -----------------------------
$placeKeys = $Dict.place.PSObject.Properties.Name | Sort-Object { $_.Length } -Descending
foreach ($jp in $placeKeys) {
...
# 見出し（長い語 → 短い語）
# -----------------------------
$labelKeys = $Dict.labels.PSObject.Properties.Name | Sort-Object { $_.Length } -Descending
foreach ($jp in $labelKeys) {
    $HtmlContent = $HtmlContent.Replace($jp, $Dict.labels.$jp)
}

# -----------------------------
# <title> の「レポート」をポルトガル語へ
# -----------------------------
$HtmlContent = $HtmlContent.Replace("レポート", "Relatório")
...
if (-not (Test-Path $OutputFile)) {
    Write-Error "ポルトガル語版HTMLが保存されていません。: $OutputFile"
    exit 1
}

Write-Host "ポルトガル語（ブラジル）版HTMLをローカル保存しました。" -ForegroundColor Green
...
$GitUrl = "https://api.github.com/repos/$Repo/contents/weather_report_now_ptbr.html"
...
$Body = @{
    message = "Update weather_report_now_ptbr.html"
...
Write-Host "ポルトガル語（ブラジル）版処理 完了！" -ForegroundColor Green
