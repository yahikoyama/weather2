...
Final AS (
    SELECT
        ...
        ROUND(
            (24 - ABS(AvgT - 21)) * 0.4 +   -- Ideal temperature range
            (60 - ABS(AvgH - 50)) * 0.2 +   -- Ideal humidity range
            (60 - DiscomfortIndex) * 0.2 +  -- Lower DI is better
            (27 - HeatIndex)       * 0.1 +  -- Lower HI is better
            (24 - WetBulb)         * 0.1,   -- Lower WetBulb is better
            2
        ) AS ComfortScore
    FROM Metrics
)

-- Output with ranking + FromDate + ToDate
SELECT
